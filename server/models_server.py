...
# from diffusers_enc import DiffusionPipelineEnc
import diffusers_enc
...
from models.tenseal import ConvNet, EncConvNet
...
def models(model_id):
    ...
    try:
        ...
        if model_id == "runwayml/stable-diffusion-v1-5":
            ...
            out["images"][0].save(f"public/images/{file_name}.png")
            result = {"path": f"/images/{file_name}.png"}

        if model_id == "runwayml/stable-diffusion-v1-5-enc":
            file_name = str(uuid.uuid4())[:4]
            text = request.get_json()["text"]
            out = pipe(prompt=text)
            dh_key_yb = out["DH_KEY_B"][0]
            with open(f"public/encimages/image_bytes_{file_name}_DH_KEY_YB={dh_key_yb}", "wb") as f:
                f.write(out["images"][0])
            result = {"path": f"public/encimages/image_bytes_{file_name}_DH_KEY_YB={dh_key_yb}"}

        # encryption image classification
        if model_id == "TenSEAL/encrypt-cnn-mnist":
            file_name = str(uuid.uuid4())[:4]
            import tenseal as ts
            import pickle
            import pdb
            pdb.set_trace()
            encoding_url = request.get_json()["image"]
            with open(encoding_url, "rb") as fp:
            ...
            serialized = ts_encoding["x_enc_serialized"]
            x_enc = ts.ckks_vector_from(context, serialized)
            windows_nb = ts_encoding["windows_nb"]
            out = pipe(x_enc, windows_nb)
            out_serialized = out.serialize()
            with open(f"public/enclabels/enc_output_encoding_{file_name}.pkl", "wb") as fp:
                pickle.dump(out_serialized, fp)
            result = {"path": f"public/enclabels/enc_output_encoding_{file_name}.pkl"}
...
if __name__ == '__main__':
    ...
    if not os.path.exists("public/encimages"):
        os.makedirs("public/encimages")
    if not os.path.exists("public/enclabels"):
        os.makedirs("public/enclabels")
