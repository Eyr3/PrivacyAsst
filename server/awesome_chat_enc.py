...
if API_TYPE == "local":
    ...
    API_ENDPOINT = f"https://api.openai.com/v1/{api_name}"  # api.openai.com
...
DH_KEY_1 = None
if config["diff_hellmen"]:
    DH_KEY_1 = config["diff_hellmen"]["key_user"]
...
def run_task(input, command, results, api_key, api_type, api_endpoint):  # , dh_key_1
    id = command["id"]
    args = command["args"]
    task = command["task"]
    deps = command["dep"]
...
def chat_huggingface(messages, api_key, api_type, api_endpoint, return_planning = False, return_results = False):  # , dh_key_1=None
    start = time.time()
    context = messages[:-1]
    ...
    while True:
        ...
        for task in tasks:
            ...
            if dep[0] == -1 or len(list(set(dep).intersection(d.keys()))) == len(dep):
                ...
                thread = threading.Thread(target=run_task, args=(input, task, d, api_key, api_type, api_endpoint))  # , dh_key_1
                thread.start()
                threads.append(thread)
...
def test():
    ...
    for input in inputs:
        ...
        chat_huggingface(messages, API_KEY, API_TYPE, API_ENDPOINT, return_planning = False, return_results = False)  # , dh_key_1=DH_KEY_1
...
def cli():
    ...
    while True:
        ...
        answer = chat_huggingface(messages, API_KEY, API_TYPE, API_ENDPOINT, return_planning=False, return_results=False)  # , dh_key_1=DH_KEY_1
...
def server():
    ...
    def tasks():
        ...
        response = chat_huggingface(messages, api_key, api_type, api_endpoint, return_planning=True)  # , dh_key_1=DH_KEY_1
    ...
    def results():
        ...
        if api_key is None or api_type is None or api_endpoint is None:
            return jsonify({"error": "Please provide api_key, api_type and api_endpoint"}) 
        response = chat_huggingface(messages, api_key, api_type, api_endpoint, return_results=True)  # , dh_key_1=DH_KEY_1
        return jsonify(response)
    ...
    def chat():
        ...
        if api_key is None or api_type is None or api_endpoint is None:
            return jsonify({"error": "Please provide api_key, api_type and api_endpoint"}) 
        response = chat_huggingface(messages, api_key, api_type, api_endpoint)  # , dh_key_1=None
        return jsonify(response)
